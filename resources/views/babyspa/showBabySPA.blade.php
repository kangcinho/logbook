...
@section('title','Data Reservasi Baby SPA')
@section('content')
  <div id="statusMsg"></div>
  <div class="card">
    <div class="card-header bg-info">
      <h5>Data Reservasi Baby SPA</h5>
    </div>

    <div class="card-body">
      <div id="loader" ></div>
      <div class="table-responsive" id="dataReservasiBabySPA"></div>
    </div>

    <div class="card-footer">
      @if(Auth::user()->can('create'))
        <a role="button" class="btn btn-primary" title="Tambah Data Reservasi Baby SPA" data-toggle="modal" data-target="#modalTambahEditReservasiBabySPA"><span class="fa fa-plus"></span> Data Reservasi Baby SPA</a>
      @endif
    </div>
  </div>

  @include('babyspa.modalDeleteDataReservasiBabySPA')
  @include('babyspa.modalTambahEditReservasiBabySPA')
@endsection

@section('additionalJS')
<script type="text/javascript" src="{{ asset('js/babyspa/babyspa_clientToCRUD.js') }}"></script>
<script type="text/javascript">
  $(document).ready(function(){
    showTabeldataReservasiBabySPA()
  })

  $(document).ajaxStart(function(){
    document.getElementById("loader").style.display = "";
    document.getElementById("dataReservasiBabySPA").style.display = "none";
  });

  $(document).ajaxStop(function(){
    document.getElementById("loader").style.display = "none";
    document.getElementById("dataReservasiBabySPA").style.display = "";
  });

  function sweetTable(){
    var toolbarTambahan = '';
    var dataTable = $('#tabelReservasiBabySPA').DataTable({
      "language": {
    ...
    toolbarTambahan += '<input class="form-control form-control-sm mx-1" type="date" name="searchRangeCheckInCheckOut1" id="searchRangeCheckInCheckOut1" value=""/>\
  ...
  function showTabeldataReservasiBabySPA(){
    $.ajax({
      type:'GET',
      url:'getBabySPA',
      success:function(data){
        ...
        var data1 = '\
        <table class="table table-hover table-bordered table-sm small" id="tabelReservasiBabySPA" cellspacing="0" width="100%"> \
            ...
            <tr'+(jsonData[indeks].konfirmasi=="1"?" class='bg-success'":"")+' id="rowKe"'+ ++no +'>\
              ...
              <td class="col-xs-1 text-right customTableResponsive">';
                @if(Auth::user()->can('update'))
                  data2 += '<a role="button" class="btn btn-sm btn-warning" title="Ubah Data Pendaftaran Baby SPA" data-toggle="modal" data-target="#modalTambahEditReservasiBabySPA" onclick="sendDataToForm(\'tabelReservasiBabySPA\', \'editTambahReservasiBabySPA\')"><span class="far fa-edit"></span></a>';
                @endif
                @if(Auth::user()->can('delete'))
                  data2 += '<a role="button" class="btn btn-sm btn-danger" title="Hapus Data Pendaftaran Baby SPA" data-toggle="modal" data-target="#modalDeleteDataReservasiBabySPA" onclick="sendDataToDelete(\'tabelReservasiBabySPA\', \'tabelDeleteBabySPA\')"><span class="far fa-trash-alt"></span></a>';
                @endif
              data2 += '</td>\
              ...
              <td class="col-xs-1 text-center customTableResponsive d-none">'+ htmlEncode(jsonData[indeks].tgl_lahir_pasien) +'</td>\
            </tr>'
          }
        data2 += '</tbody> \
        </table>';

        $('#dataReservasiBabySPA').html(data1+data2);
        sweetTable();
      }
    });
  }
