      ...
      success:function(data){
        ...
        <table class="table table-striped table-bordered table-sm small" id="tabelReservasiRadiologi" cellspacing="0" width="100%"> \
          <thead class="bg-light">\
            <tr>\
              ...
              <th class="col-xs-1 text-center customTableResponsive">Petugas Radiologi</th>\
        ...
        var dataTambahan = '';
          for(indeks in jsonData){
            dataTambahan = ''+'&hobayu&'+jsonData[indeks].slug +'&hobayu&'+ (jsonData[indeks]['pasien']?jsonData[indeks]['pasien'].slug:'') +'&hobayu&'+ jsonData[indeks].tgl_ditindak +'&hobayu&'+ jsonData[indeks].waktu_ditindak +'&hobayu&'+ (jsonData[indeks]['pasien']?jsonData[indeks]['pasien'].no_rm:'') +'&hobayu&'+ '' +'&hobayu&'+ jsonData[indeks].nama_pasien+ '&hobayu&'+ jsonData[indeks].tlp_pasien+ '&hobayu&'+ jsonData[indeks].tgl_lahir_pasien+ '&hobayu&'+ jsonData[indeks].alamat_pasien +'&hobayu&'+ jsonData[indeks].dokter_pengirim +'&hobayu&'+ jsonData[indeks].dokter_penindak +'&hobayu&'+ jsonData[indeks].jenis_tindakan +'&hobayu&'+ jsonData[indeks].petugas_radiologi +'&hobayu&'+ jsonData[indeks].id_bpjs_pasien +'&hobayu&'+ jsonData[indeks].kelas_bpjs_pasien +'&hobayu&'+ '' +'&hobayu&'+ jsonData[indeks].konfirmasi;
            data2 += '\
            <tr>\
              <td class="col-xs-1 text-center customTableResponsive">'+ tanggal(jsonData[indeks].tgl_ditindak) +' '+ jsonData[indeks].waktu_ditindak +' '+ (jsonData[indeks].konfirmasi=="1"?'<i class="fas fa-check"></i>':"") +'</td>\
              ...
              <td class="col-xs-1 text-left customTableResponsive">'+ jsonData[indeks].dokter_pengirim +'</td>';
              if(jsonData[indeks].surat_rujukan != ''){
                data2 += '<td class="col-xs-1 text-center customTableResponsive"><a role="button" data-toggle="modal" data-target="#modalShowImage" class="btn btn-outline-info btn-sm" data-tambahan="{!! asset('DataRadiologi') !!}'+ '/' +(jsonData[indeks]['pasien']?jsonData[indeks]['pasien'].no_rm:'') + '/' + jsonData[indeks].tgl_ditindak+ '/' + jsonData[indeks].surat_rujukan + '"><span class="fa fa-search"></span></a></td>';
              }else{
                data2 += '<td></td>';
              }
              data2 += '<td class="col-xs-1 text-left customTableResponsive">'+ jsonData[indeks].dokter_penindak +'</td>\
              <td class="col-xs-1 text-left customTableResponsive">'+ jsonData[indeks].jenis_tindakan +'</td>\
              <td class="col-xs-1 text-left customTableResponsive">'+ jsonData[indeks].petugas_radiologi +'</td>\
